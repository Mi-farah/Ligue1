"""
Simplified service class for handling plane journey calculations.

This service provides functionality to:
- Calculate plane routes between stadiums using Google Maps API
- Compare plane vs car emissions for optimal route selection
...
from backend.global_variables import (
    AIRPORT_CACHE_FILENAME,
    AUTO_CAR_EMISSION_FACTOR,
    DATA_PATH,
    FLIGHT_EMISSIONS_FILENAME,
...
class PlaneTrajetService(BaseTransportService):
    ...
    def is_real_airport(self, place_name: str) -> bool:
        ...
        return any(
            keyword.lower() in place_name.lower() for keyword in self.airport_keywords
        )
    ...
    def calculate_route(
        ...
        departure_airport = self.get_nearest_airport(
            club_name=departure, lat=departure_coords[0], lon=departure_coords[1]
        )
        arrival_airport = self.get_nearest_airport(
            club_name=arrival, lat=arrival_coords[0], lon=arrival_coords[1]
        )
        ...
        stadium_to_airport_origin = self._format_coordinates(
            departure_coords[0], departure_coords[1]
        )
        stadium_to_airport_dest = self._format_coordinates(
        ...
        airport_to_stadium_origin = self._format_coordinates(
            arrival_airport["latitude"], arrival_airport["longitude"]
        )
        airport_to_stadium_dest = self._format_coordinates(
        ...
        added_details = ""
        added_details += (
            "No autocar departure route found"
            if not distance_autocar_dep
            else "departure autocar route found"
        )
        added_details += (
            "- No autocar arrival route found"
            if not distance_autocar_arr
            else "- arrival autocar route found"
        )
        ...
        plane_emission = (
            self.jet_fuel_to_co2
            * fuel_consumption
            * self.multiplication_factor_other_than_fuel
        )
        autocar_emission = (
            self.autocar_emission_factor
            * self.number_of_passengers
            * (distance_autocar_dep + distance_autocar_arr)
        )
        ...
        return RouteData(
            departure=departure,
            arrival=arrival,
            travel_time_seconds=total_time * 2,
            distance_km=total_distance * 2,
            emissions_kg_co2=total_emissions * 2,
            transport_type="plane",
            route_details={
                "one_way_flight_distance_km": flight_distance,
                "one_way_flight_time_seconds": flight_time,
                "one_way_autocar_distance_km": distance_autocar_dep
                + distance_autocar_arr,
                "one_way_autocar_time_seconds": time_autocar_dep + time_autocar_arr,
                "one_way_fuel_consumption_kg": fuel_consumption,
                "one_way_plane_emission_kg_co2": plane_emission,
                "one_way_autocar_emission_kg_co2": autocar_emission,
                "added_details": added_details,
                "travel_steps": [
                    {
                        "step_type": "autocar_departure",
                        "from": departure,
                        "from_coords": [departure_coords[0], departure_coords[1]],
                        "to": departure_airport.get("airport_name", ""),
                        "to_coords": [
                            departure_airport.get("latitude", None),
                            departure_airport.get("longitude", None),
                        ],
                        "distance_km": distance_autocar_dep,
                        "travel_time_seconds": time_autocar_dep,
                    },
                    {
                        "step_type": "flight",
                        "from": departure_airport.get("airport_name", ""),
                        "from_coords": [
                            departure_airport.get("latitude", None),
                            departure_airport.get("longitude", None),
                        ],
                        "to": arrival_airport.get("airport_name", ""),
                        "to_coords": [
                            arrival_airport.get("latitude", None),
                            arrival_airport.get("longitude", None),
                        ],
                        "distance_km": flight_distance,
                        "travel_time_seconds": flight_time,
                    },
                    {
                        "step_type": "autocar_arrival",
                        "from": arrival_airport.get("airport_name", ""),
                        "from_coords": [
                            arrival_airport.get("latitude", None),
                            arrival_airport.get("longitude", None),
                        ],
                        "to": arrival,
                        "to_coords": [arrival_coords[0], arrival_coords[1]],
                        "distance_km": distance_autocar_arr,
                        "travel_time_seconds": time_autocar_arr,
                    },
                ],
            },
        )
    ...
    def run_complete_analysis(
        self, output_filename: str = FLIGHT_EMISSIONS_FILENAME
    ) -> None:
        """
